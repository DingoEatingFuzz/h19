...
export class EBB {
  ...
  public lowlevelMove(
    ...
    const [initialRate1, deltaR1] = this.axisRate(
      stepsAxis1,
      initialStepsPerSecAxis1,
      finalStepsPerSecAxis1
    );
    const [initialRate2, deltaR2] = this.axisRate(
      stepsAxis2,
      initialStepsPerSecAxis2,
      finalStepsPerSecAxis2
    );
    return this.command(
  ...
  public async waitUntilMotorsIdle(): Promise<void> {
    while (true) {
      const [, commandStatus, _motor1Status, _motor2Status, fifoStatus] = (await this.query(
        "QM"
      )).split(",");
  ...
  public async supportsLM(): Promise<boolean> {
    if (typeof this.cachedSupportsLM === "undefined") {
      ...
      this.cachedSupportsLM =
        major > 2 || (major === 2 && minor > 5) || (major === 2 && minor === 5 && patch >= 3);
    }
  ...
  private axisRate(
    steps: number,
    initialStepsPerSec: number,
    finalStepsPerSec: number
  ): [number, number] {
